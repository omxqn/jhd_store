...
export default function NewProductPage() {
    ...
    const [form, setForm] = useState({
        id: "", name: "", category: "Thobes", price: "", old_price: "",
    ...
    // Auto-suggest ID on load + fetch categories + necklines
    useEffect(() => {
        fetch("/api/admin/products?suggest_id=1")
            .then(r => r.json())
            .then(d => { if (d.next_id) setForm(prev => ({ ...prev, id: d.next_id })); })
            .catch(() => { });
    ...
    // Image upload
    const handleFileUpload = async (files: FileList | null, insertIdx?: number) => {
        if (!files?.length) return;
        const idx = insertIdx ?? images.length;
        setUploadingIdx(idx);
        ...
        try {
            ...
            if (data.url) setImages(prev => { const a = [...prev]; a.splice(idx, 0, data.url); return a; });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!form.id || !form.name || !form.price || images.length === 0) {
            toast.error("ID, Name, Price, and at least one image are required");
            return;
        }
        setLoading(true);
        try {
            ...
            else { const d = await res.json(); toast.error(d.error || "Failed to create product"); }
            ...
            <form onSubmit={handleSubmit}>
                            ...
                            {images.map((url, i) => (
                                <div key={i} style={{ position: "relative", borderRadius: "8px", overflow: "hidden", aspectRatio: "1", border: "1px solid var(--admin-border)" }}>
                                    ...
                                    <button type="button" onClick={() => removeImage(i)}
                                        style={{ position: "absolute", top: "4px", right: "4px", background: "rgba(0,0,0,0.7)", color: "#fff", border: "none", borderRadius: "50%", width: "22px", height: "22px", cursor: "pointer", fontSize: "0.75rem", display: "flex", alignItems: "center", justifyContent: "center" }}>
                                        ×
                                    </button>
                ...
                <div style={sectionStyle}>
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: "0.75rem", cursor: "pointer" }}>
                        <input type="checkbox" id="ms" checked={form.most_selling as boolean} onChange={f("most_selling")} style={{ width: 18, height: 18, accentColor: "var(--admin-primary)" }} />
                        <div>
